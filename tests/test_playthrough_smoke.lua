...
local function bootToPlaying()
    ...
    -- Skip planet selection (select Erebus and advance to requisition unlocks)
    PlanetSelect.confirm()
    T.eq(GameState.phase, 'requisition_unlocks', 'planet confirm enters requisition_unlocks phase')

    -- Skip requisition unlocks (press ESC to continue)
    local ReqPanel = require('src.ui.requisition_panel')
    ReqPanel.keypressed('escape')
    love.update(0.05)
    T.eq(GameState.phase, 'drafting', 'requisition unlocks done enters drafting phase')

    -- Deploy crew (advances to requisition picks)
    ColonistSelect.deploy()
    T.eq(GameState.phase, 'requisition_picks', 'crew deployment enters requisition_picks phase')

    -- Skip requisition picks (press ESC to continue)
    ReqPanel.keypressed('escape')
    love.update(0.05)
    T.eq(GameState.phase, 'world_map', 'requisition picks done enters world_map phase')

    -- Confirm landing zone (advances to starting)
    local WorldMap = require('src.ui.world_map')
    WorldMap.confirm()
    T.eq(GameState.phase, 'starting', 'world map confirm enters starting phase')
    love.update(0.05)
