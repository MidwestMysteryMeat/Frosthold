...
**Last updated:** 2026-07-29 by Claude Code (UI overhaul — panel manager, layout helpers, gift/trade fix; suite 529/529 green)
...
|---|------|-----------|-------|-------|
| Task 14 | UI overhaul — panel manager, shared layout helpers, gift/trade fix | 2026-07-29 | Claude Code | New `src/ui/panel_manager.lua`: one major panel open at a time, input routed to the focused panel only, ESC closes the top panel. Fixes the reported defect where the roster showed through the diplomacy screen and the panel drawn *underneath* ate every click (main.lua asked panels in a fixed order and each consumed input while visible), which is why gift/trade looked dead. New `src/ui/ui_layout.lua`: measured truncation (UTF-8 safe), label/value columns, auto-sized buttons, nested clipping, scroll clamps, `Layout.BOTTOM_RESERVE` so no panel footer hides under the bottom toolbar. factions.sendGift: checked only GameState.resources (0 for cores/metal/components now goods are items) and ignored SNet.withdraw's return, so gifts of stored goods were refused and gifts from the counter were free — now checks storage+counter and drains storage then counter. Toasts moved to draw after panels (`UI.drawToast`); they were rendered underneath the panel that produced them. Debug panel moved F7 -> F11 (F7 belongs to the logistics overlay). Panel sweep: faction/colony/medical/equip/taming/laws/farm/research/policy/doctrine/quest/goals/trade/expedition — collisions, clipped buttons, missing scroll and unclipped lists; expedition_view could compute a negative scissor height and crash. Byte-index slicing replaced with codepoint-safe helpers across the UI (backspace handlers included). New suites: test_ui_layout (39), test_panel_manager (15), test_factions_gift (10). Suite 529/529. Dev tool: `--uishots` + tools/collect_uishots.ps1 screenshots every panel (images gitignored). |
